...
NO_ECHO=$($OLD_BINARY tx distribution fund-community-pool 100000000$TOKEN_DENOM --keyring-backend test --chain-id $CHAIN_ID --home $CHAIN_HOME --from wallet1 -y)
sleep 1
COMMUNITY_POOL_AMOUNT=$($OLD_BINARY query distribution community-pool --home $CHAIN_HOME --output=json | jq ".pool[0].amount")
echo "COMMUNITY_POOL_AMOUNT $COMMUNITY_POOL_AMOUNT"
# Check if community pool has enough funds
if [[ "$COMMUNITY_POOL_AMOUNT" == "1000000.000000000000000000" ]]; then
    echo "Community pool not funded"
    exit 1
fi
...
COMMUNITY_POOL_TOKENS=$($NEW_BINARY query distribution community-pool --home $CHAIN_HOME --output=json | jq ".pool | length")
if [[ "$COMMUNITY_POOL_TOKENS" != "1" ]]; then
    echo "Token not burnt"
    exit 1
fi
