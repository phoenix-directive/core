...
func CreateUpgradeHandler(
	...
	return func(ctx context.Context, plan upgradetypes.Plan, vm module.VersionMap) (module.VersionMap, error) {
		...
		} else if sdkCtx.ChainID() == "pisco-1" {
		...
		for _, coin := range communityPoolCoins {
			if !slices.Contains(assetsToBurnList, coin.Denom) {
				continue
			}

			// Update community pool to reflect the burn
			communityPool, _ := k.DistrKeeper.FeePool.Get(ctx)
			...
			k.DistrKeeper.FeePool.Set(ctx, communityPool)
